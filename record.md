...
文章中使用的方法为Mutual Nearest Neighbor (MNN)，这是由于文章的预处理流程依赖于Monocle3。这个方法会在局域中寻找最近邻，因此在处理不同batch间相似性/重合较高时会较有优势，并且会给出一个“修正后的表达矩阵”。此外，这种方法导致去样本的作用（混合的效果）不那么明显，一定程度上保留了时间信息，有利于后续的轨迹分析。
